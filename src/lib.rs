...
impl LoadBar {
    #[new]
    #[pyo3(signature = (label, finish, format_str, ac_clr, br_clr, ex_clr, smoothing=0.3))]
    fn py_new(label: &str, finish: f64, format_str: &str, ac_clr: &str, br_clr: &str, ex_clr: &str, smoothing: f64) -> Self {
        let now = Instant::now();
        Self {
    ...
    fn update(&mut self, new_prog: f64) {
        ...
        // 1. Render strings as OWNED variables first to avoid E0716
        ...
        let time_s = format!("{:02}:{:02}<{:02}:{:02}", (elapsed/60.0) as u64, (elapsed%60.0) as u64, (eta/60.0) as u64, (eta%60.0) as u64).with(self.ex_clr).to_string();

        let mut output = self.format_str.clone()
            .replace("{action}", &label_s)
            .replace("{percent}", &pct_s)
            .replace("{speed}", &speed_s)
            .replace("{elapsed}", &time_s)
            .replace("{br1}", &"[".with(self.br_clr).to_string())
            .replace("{br2}", &"]".with(self.br_clr).to_string());

        // 2. Width Calculation Logic (Fixed Borrowing)
        let (width, _) = terminal::size().unwrap_or((80, 20));
        let base_for_width = output.replace("{bar}", ""); // Owned string
        let re_ansi = Regex::new(r"\x1b\[[0-9;]*[mK]").unwrap();
        let stripped = re_ansi.replace_all(&base_for_width, ""); // Borrows from base_for_width
    ...
    fn finish(&self) { println!(); }
}

#[pymodule]
fn vibe_loadbar_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // This makes the 'LoadBar' class available in the compiled module
    m.add_class::<LoadBar>()?;
    Ok(())
}